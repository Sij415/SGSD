...
$query = "SELECT * FROM Settings WHERE Setting_Key = 'AdminSignUpEnabled' OR Setting_Key = 'SignUpEnabled' OR Setting_Key = 'MaxSignUps'";
$stmt = $conn->prepare($query);
$stmt->execute();
...
<div class="admin-restrictions">
    ...
    <form action="" method="POST">
        ...
        <input type="number" class="form-control" id="signup-amount" name="signup_amount" placeholder="<?php echo htmlspecialchars($settings['MaxSignUps'] ?? 'Set'); ?>" value="<?php echo htmlspecialchars($settings['MaxSignUps'] ?? ''); ?>" style="width: 150px;">
    </div>
        </div>
        <div class="d-flex justify-content-between align-items-start p-3">
            <div class="admin-restrictions-title d-flex flex-column mb-3">
